...
/*function repeatedString(string, number) {
  ...
  for (let i = 0; i < number / string.length; i++) {
    repeatWord.push(string);
  }
  return repeatWord.join("").slice(0, number).match(/a/g).length;
}
...
function repeatedString(string, number) {
  return ((string.split("a").length - 1) * Math.floor(number / string.length)) + (string.slice(0, (number % string.length)).split("a").length - 1);
}
